...
{  
  ...
  char buf[BSIZE];
  struct dinode din;
  int kernelCodeBlockskSize;
  
  int kernelFD = open(argv[3], O_RDWR);
  kernelCodeBlockskSize= (   (lseek(kernelFD, 0, SEEK_END) +1)/BSIZE  )+1;
  lseek(kernelFD, 0, SEEK_SET);
  
  printf("\n kernelSize: %d \n", kernelCodeBlockskSize);
...
for(i = 0; i < FSSIZE ; i++) //nullify all fs.img blocks
  ...
  wsectAbs(0, buf);								//write the buffer(mbr) into the first block
  
/*-------------------finish copying mbr to block 0-------------------*/
  
	
	
/*-------------------the next code is copying kernel code to fs.img right after bootloader  code(block 0)-------------------*/
  ...
  //kernelCodeBlockskSize+1
  for (i =1; i<kernelCodeBlockskSize;  i++){
    
    memset(buf, 0, sizeof(buf));	//nullify buf: a buffer the size of a block allocated on the stack    
    
    readed=read(kernelFD, &buf, BSIZE);
    if (readed < 0)
    {     
      perror(argv[3]);
      exit(1);
    ...
    wsectAbs(i, buf);	//write the buffer(block of kernel code) into the first block
